...
class Cleaning(BaseModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field in ["status", "scheduled_start_dt"]:
            try:
                field_name = field
    ...
    def get_main_invoice(self):
        return self.cleaninginvoice_set.filter(is_main=True).last()
    ...
    def get_payment_status(self):
        ...
        if invoice:
            if invoice.is_paid:
                ...
                is_paid = invoice.check_if_is_paid_now()
        else:
            is_paid = False
        is_paid = "Paid" if is_paid else "Waiting for the payment"
        return is_paid
