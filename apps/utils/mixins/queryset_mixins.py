...
from apps.cleaners.models import SchedulePeriod
...
class CleaningsMixin:
    ...
    def get_queryset(self):
        ...
        if user.is_general_admin:
            ...
            return user.get_cleanings_from_assigned_tickets()
        else:
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated:
            if user.is_general_admin:
                ...
                support_tickets = user.get_assigned_tickets()
                user_ids = list(support_tickets.values_list("user_id", flat=True))
                return qs.filter(client__id__in=user_ids)
            else:
                return self.model.objects.none()
        else:
...
class SpecialCleaningRequestsMixin:

    def get_queryset(self):
        user = self.request.user
        if user.is_manager:
            return user.company.get_special_requests()
        else:
            return self.model.objects.none()
